"""Per-expiry detail dump for manual inspection.

For every expiry in data/, write a CSV with minute-by-minute snapshots
from 14:30 to 15:30 IST showing what the bot would have seen:

  - spot price
  - frozen ATM strike (computed once at 14:50)
  - close + OI for ATM-50, ATM, ATM+50  (CE and PE both)
  - OI change vs 14:30 baseline (shows the writers' shift in real time)

Output: reports/expiry_detail/<expiry_token>.csv (one file per expiry)
        reports/expiry_detail/_index.csv (master list with metadata)

Use this to eyeball what was happening on each day:
  - Was OI flipping toward put-writing late in the day?
  - Did spot move sharply after 2:50 PM?
  - Did CE or PE explode in the final 30 mins?
"""
...
def dump_one(expiry_token: str, expiry_date, spot_df: pd.DataFrame, out_dir: Path) -> dict:
    ...
    # Baseline OI for change calculation: open_interest at 14:30
    baselines = {}
    ...
    for ts in minutes:
        ...
        row = {"ts": ts.strftime("%H:%M"), "spot": spot}
        for s in strikes:
            label = "ATM" if s == atm else ("ATM-50" if s < atm else "ATM+50")
            for opt in ("CE", "PE"):
                df = legs.get((s, opt))
                ...
                row[f"{label}_{opt}_close"] = round(close, 2) if close is not None else ""
                row[f"{label}_{opt}_OI"] = int(oi) if oi is not None else ""
                row[f"{label}_{opt}_OI_chg_vs_1430"] = int(oi_chg) if oi_chg is not None else ""
        rows.append(row)

    out_path = out_dir / f"{expiry_token}.csv"
    pd.DataFrame(rows).to_csv(out_path, index=False)

    # Compute summary metrics for this expiry
...
def main():
    out_dir = base.REPORTS_DIR / "expiry_detail"
    ...
    print(f"Dumping per-expiry detail for {len(expiries)} expiries -> {out_dir}/\n")
